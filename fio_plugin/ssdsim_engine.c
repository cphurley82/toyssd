...
static int ssdsim_getevents(struct thread_data* td, unsigned int min,
  ...
  if (max > td->o.iodepth) max = td->o.iodepth;
...
static struct io_u* ssdsim_event(struct thread_data* td, int event) {
  struct ssdsim_state* st = (struct ssdsim_state*)td->io_ops_data;
  if (!st || (unsigned int)event >= st->nr_events) return NULL;
...
struct ioengine_ops* get_ioengine(void) { return &ioengine; }
