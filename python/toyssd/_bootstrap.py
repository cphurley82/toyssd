...
def _prepend_env_paths(var: str, paths: Sequence[pathlib.Path]) -> None:
  ...
  if not parts:
    return
  if existing:
    parts.extend(x for x in existing.split(":") if x)
  dedup: list[str] = []
  seen = set()
  for item in parts:
    if item not in seen:
...
import pysysc  # noqa: E402  # pylint: disable=wrong-import-position
import cppyy  # noqa: E402  # pylint: disable=wrong-import-position
...
class ToySSD:
  ...
  def run_for(self, *, ns: int | None = None, us: int | None = None, ms: int | None = None) -> None:
    if ns is not None:
